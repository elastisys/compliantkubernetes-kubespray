...
1. Pull the latest changes, switch to the correct branch and update Kubespray submodule:
    ...
    ```bash
    git pull
    git switch -d v2.30.x-ck8sx
    git submodule sync
    git submodule update --init --recursive
    ```

1. Run `bin/ck8s-kubespray upgrade both v2.30 prepare` to update your config.
    ...
    >
    > ```bash
    > bin/ck8s-kubespray upgrade wc v2.30 prepare
    > ```
