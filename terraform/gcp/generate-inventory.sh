...
for cluster in sc wc; do
  MASTERS=$(jq -r '.'"${cluster}"'_master_ips.value | to_entries[]'  <(echo "${TF_OUT}"))
  WORKERS=$(jq -r '.'"${cluster}"'_worker_ips.value | to_entries[]'  <(echo "${TF_OUT}"))
  mapfile -t MASTER_NAMES < <(jq -r '.key'  <(echo "${MASTERS}"))
  mapfile -t WORKER_NAMES < <(jq -r '.key'  <(echo "${WORKERS}"))

  API_LB=$(jq -r '.'"${cluster}"'_control_plane_lb_ip_address.value' <(echo "${TF_OUT}"))
