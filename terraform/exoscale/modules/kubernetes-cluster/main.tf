...
resource "exoscale_compute" "worker" {
  ...
  user_data = templatefile(
    "${path.module}/templates/worker-cloud-init.tmpl",
    {
      ...
      ceph_enabled              = each.value.provider_settings != null && each.value.provider_settings.ceph_partition_size != 0
      ceph_partition_size       = each.value.provider_settings != null ? each.value.provider_settings.ceph_partition_size : 0
    }
  )
}
